...
proposera une solution unique sur le marché\todo{YC : pas sûr que bonne idée de parler du marché. Plus de détails sur comment l'utilisation de l'outil peut aider (problème d'ordonnancement notamment de workflow)}.\\
...
\todo{YC : avant de commencer, une sous section qui décrit ce qu'est le Cloud, et le contexte pour nous : solution de stockage ?}Cette étude peut être divisée en deux parties. Tout d’abords, nous avons
...
de réaliser des tests de performances sur des “Cloud”\todo{YC : marques de citations ou pas ? Attention, clouds virtuels et non de stockage, Google développe quelque chose...}, l'abscence de résultats
pouvant justifier\todo{YC : ça donne toujours l'impression que vous aviez vraiment bcp envie de coder, et qu'il fallait trouver un truc, n'importe quoi, qui permette de justifier le passage à l'acte...} le développement de notre
propre application.
...
populaires, nous avons décidé de le choisir
%: il apporte probablement des services supplémentaires. Nous pensons qu’
car il est également intéressant de
...
Performance\todo{YC : il manque des références sur les outils. Pareil pour Owncloud avant, Google Drive. Il faut une section Bibliographie, ou des foottexts avec les URLs}, est une solution propriétaire payante. Parmi de nombreux outils,
...
Nous les avons comparés à notre projet baptisé \KYD (Know Your Data).\todo{YC : cette phrase doit explictement introduire le tableau auquel vous ne faîtes toujours pas référence. Vérifiez pour les autres tableaux, et référence dans le texte, voire explication de ce qu'on y trouve}
...
bout\todo{YC : utilisez vos propres termes. Ajoutez de bout en bout tel quel n'a pas vraiment de sens. On s'intéresse où sont le client et le serveur, soit aux bouts des communications, pas où se trouve l'utilisateur..}), la date du test, les hébergeurs à tester, ainsi que le type de transfert
...
requêter la base \textit{MongoDB} et renvoyés une solution\todo{YC : phrase bizarre} si elle existe. Dans
...
\todo{YC : le shéma est vraiment trop gros vu la simplicité de ses composants.}
...
serait par exemple possible de vérifier l'efficacité des comptes premium qui
...
stockés dans une base de données, soient plus facile d'accès\todo{YC : cad ?}.
...
tests a permis d'obtenir ces résultats\todo{YC : si vous avez des écarts-types, ça peut être bien d'en parler, par rapport à la reproductibilité à un instant donné}.
...
\todo{YC : le titre dans le png est à retirer du coup, car dans le caption}
...
\todo{YC : heure de la journée, mieux que tranche(s) horaire (avec une faute)}
...
\todo{YC : quid des pics de latence ?}
...
stage a trouvé sa place.\todo{YC : vous ne saviez pas vraiment quoi écrire, hein ? Dites que nous sommes dans un cadre big data (grosses et/ou nombreuses), et que pour obtenir des performances, il faut minimiser le temps d'accès aux données, et donc notamment réduire les temps de transferts en choisissant une source de données proche de la destination, et que cela peut impliquer des mécanismes de préfect, de recopie/migration de données afin de disposer de plusieurs sources possibles. Reste à sélectionner la source, votre outil arrive là}\\ 
...
sélectionner les meilleures solutions "\textit{Cloud}"\todo{YC : homogénéisez partout : cloud, ou italique ou coté. Perso, cloud tel quel est très bien}.\\
...
future\todo{YC : ça, c'est un lanceur de questions pour l'oral, avec explication de votre positionnement}.\\

\todo{YC : ce qui *peut* manquer : bibliographie, avec les références des outils que vous avez utilisés. Le lien github du projet (que le projet soit public ou pas) et quelques informations sur le projet comme le nombre de lignes, etc. Une page de garde éventuellement avec le logo de l'Univ, etc., Le nom des encadrants au cas où on devienne populaire (et accessoirement car ça permet de situer le travail)}
...
\item Travis.cl - Intégration continue
\item Ip-api et Telize - API de géolocalisation
\item Execo - Gestionnaire de processus unix
