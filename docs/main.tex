...
que le type de transfert (\textit{upload} ou \textit{download})\todo{YC : regrouper protocole et type de transfer} pour sauvegarder les
...
Suite à de nombreux tests\todo{YC : décrivez calmement le protocol expérimental.
 Quels sont les paramètres communs examinés à toutes les expériences, quelles sont les caractéristiques qui ont changé, combien de tests ont été fait (parler d'intervalle si besoin).}, nous avons pu obtenir différentes courbes traduisant
...
différences selon le moment de la journée ou selon le jour de la semaine\todo{YC : ettayez}.\\
...
\textit{download} avec une taille de fichier différente puis la moyenne\todo{YC : flou. Moyenne sur combien ?} de ces
...
nettement inférieures à celles de ses concurrents\todo{YC : du bon commentaire. J'en veux d'autres ! ;D}.

\section{Conclusion}

Durant ce projet, nous avons donc implémenté\todo{YC : ouch. CE qu'il ne faut pas faire...} un outil permettant d'estimer la
